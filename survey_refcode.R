...
# ---------- SURVEY DATAFRAME
header_main_surveyx <- scan("~/Google Drive/1)Thesis Online/Survey_Data_R/csv_data/Social_Privacy_on_Facebook.csv", nlines = 1, what = character (), sep = ",") # Extract only headers
data_main_surveyx <- read.csv("~/Google Drive/1)Thesis Online/Survey_Data_R/csv_data/Social_Privacy_on_Facebook.csv", skip = 2, header = FALSE, sep = ",") # Extract only data, w/out headers ## If the file is .txt, use "read.table()"; if not, use "read.csv()"
...
raw_survey <- read.csv("~/Google Drive/1)Thesis Online/Survey_Data_R/csv_data/Social_Privacy_on_Facebook.csv", header = TRUE)

##Write CSV file (for external visualization needing clean data like Tableau, PowerBI...)
# write.csv(Survey, file="~/Google Drive/1)Thesis Online/Survey_Data_R/csv_data/Social_Privacy_on_Facebook_final.csv")
